...
private:
	...
	vector<_Parent_Block_Info>::iterator FindParentBlock(Block& parent) {
		return std::lower_bound(_parent_blocks.begin(), _parent_blocks.end(), &parent);
	}
...
public:
	...
	void SetPosition(Block& parent, Point position) {
		FindParentBlock(parent)->iterator->child_position = position;
	}
	void SetWidth(Block& parent, uint width) {
		FindParentBlock(parent)->iterator->child_width = width;
	}
};